...
class MqttMessagePublish extends MqttMessage {
  ...
  /// Constructors
  MqttMessagePublish.setOptions(String topic, String payload,
      [num msgID = 1, int QoS = 0, int retain = 0])
      : this._topic = topic,
        ...
        super.setOptions(
            PUBLISH,
            (QoS > 0)
                ? (6 + UTF8.encode(topic).length + UTF8.encode(payload).length)
  ...
  num decodeVariableHeader(List<int> data) {
    ...
    if (QoS > 0) {
      // QOS 1 and 2 include a message ID
      _msgID_MSB = data[pos++];
      _msgID_LSB = data[pos++];
    }

    _payloadPos = 2 +
        pos; // position for the 1st payload character = 2 (fixed header length) + pos (variable header length)
  ...
  decodePayload(List<int> data) {
    ...
    if (payloadLen <= data.length) {
      ...
      print(
          "WARNING: Payload is truncated - Characters received: ${data.length} - expected: ${payloadLen} ");
