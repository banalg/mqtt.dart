...
class MqttClient<E extends VirtualMqttConnection> {
   ...
   */
  MqttClient(E mqttConnection,
      {String clientID: '', num qos: 0x0, bool cleanSession: true})
  ...
  void setWill(String topic, String message,
      {int qos: QOS_0, bool retain: false}) {
  ...
  */
  Future<Null> connect({Function onConnectionLostCallback, Iterable<String> protocols}) async {
    _connack = new Completer();

    if (onConnectionLostCallback != null)
      onConnectionLost = onConnectionLostCallback;

    var socket = await _mqttConnection.connect(protocols:protocols);
   ...
   */
  Future<MqttMessageSuback> subscribe(
      String topic, int QoS, onSubscribeDataCallback,
      [num messageID = 1]) {
    print("Subscribe to $topic - QoS: $QoS - Message ID: $messageID");
    Completer suback = _addMessageToComplete(messageID, QOS_1,
        new MqttMessageSuback()); // send as QOS_1 as a SUBACK is expected

    onSubscribeData = onSubscribeDataCallback;

    MqttMessageSubscribe m =
        new MqttMessageSubscribe.setOptions(topic, messageID, QoS);
    _mqttConnection.sendMessageToBroker(m, debugMessage);
  ...
  Future<MqttMessageUnsuback> unsubscribe(String topic, num messageID) {
    ...
    Completer unsuback = _addMessageToComplete(messageID, QOS_1,
        new MqttMessageUnsuback()); // send as QOS_1 as a UNSUBACK is expected

    MqttMessageUnsubscribe m =
        new MqttMessageUnsubscribe.setOptions(topic, messageID);
    _mqttConnection.sendMessageToBroker(m, debugMessage);
  ...
  Future<MqttMessagePuback> publish(String topic, String payload,
      [num messageID = 1, int QoS = 0, bool retain = false]) {
    int retainFlag = (retain) ? 1 : 0;
    if (verbosity > 1) {
      print(
          "Publish $topic: $payload (ID: $messageID - QoS: $QoS - retain: $retainFlag)");
    }

    Completer puback = _addMessageToComplete(
        messageID, QoS, new MqttMessagePuback.initWithMessageID(messageID));

    // publish the message
    MqttMessagePublish m = new MqttMessagePublish.setOptions(
        topic, payload, messageID, QoS, retainFlag);
    _mqttConnection.sendMessageToBroker(m, debugMessage);
   ...
   */
  Completer _addMessageToComplete(
      num messageID, int QoS, MqttMessageAssured m) {
    ...
    if (QoS > 0) {
      ...
      // QOS 0 - no puback will be received
      ack.complete(m);
    }
  ...
  void _openSession() {
    ...
    MqttMessageConnect m =
        new MqttMessageConnect.setOptions(_clientID, _qos, _cleanSession);
  ...
  List<int> _processMqttMessage(data) {
    ...
    return (data.length > msgProcessedLength)
        ? data.sublist(msgProcessedLength)
        : null;
  }
  ...
  int _handlePublish(data) {
    ...
    if (m.QoS > 0) {
      MqttMessageAssured mAck = ((m.QoS == 1)
          ? new MqttMessagePuback.initWithPublishMessage(m)
  ...
  int _handlePubRel(data) {
    MqttMessagePubcomp m = new MqttMessagePubcomp.initWithPubRelMessage(
        new MqttMessagePubrel.decode(data, debugMessage));
    _mqttConnection.sendMessageToBroker(m, debugMessage);
    _resetTimer();
  ...
  /**
     ...
     */
  int _handlePingReq() {
    _mqttConnection.sendMessageToBroker(
        new MqttMessagePingResp(), debugMessage);
    _resetTimer();
    return 2;
  }
