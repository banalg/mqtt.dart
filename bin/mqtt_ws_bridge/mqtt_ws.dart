...
class MqttWebSocketBridge {
  ...
  onConnection(WebSocket ws) {
    ...
    print(
        "New webSocket connection. Connecting to mqtt broker ${_mqttHost}:${_mqttPort}");
...
void main() {
  ...
  HttpServer.bind('127.0.0.1', bridgeOptions.listeningPort).then(
      (HttpServer server) {
    print('listening for connections on ${bridgeOptions.listeningPort}');

    var wsBridge =
        new MqttWebSocketBridge(bridgeOptions.mqttHost, bridgeOptions.mqttPort);
