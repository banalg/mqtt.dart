...
class MqttSubOptions {
  ...
  ArgParser argParser() {
    ...
    parser.addOption('topic',
        abbr: 't', help: 'topic', callback: (t) => _topic(t));
...
main(List<String> args) {
  ...
  // Create MqttClient
  MqttClient<MqttConnectionIOSocket> c = new MqttClient(
      new MqttConnectionIOSocket.setOptions(
...
void subscribe(MqttClient c, MqttSubOptions mqttOptions) {
  c
      .subscribe(mqttOptions.topic, mqttOptions.QoS, (t, d) => print("[$t] $d"))
      .then((s) => print(
          "Subscription done - ID: ${s.messageID} - Qos: ${s.grantedQoS}"));
  ;
}
