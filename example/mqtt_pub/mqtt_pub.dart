...
class MqttPubOptions {
  ...
  ArgParser argParser() {
    ...
    parser.addOption('mqttUrl',
        abbr: 'U',
        defaultsTo: 'ws://test.mosquitto.org:8080',
    ...
    parser.addOption('user',
        abbr: 'u', help: 'username', callback: (u) => user = u);

    parser.addOption('password',
        abbr: 'p', help: 'password', callback: (p) => password = p);
...
main(List<String> args) async {
  ...
  // Create MqttClient
  MqttClient c = new MqttClient(mqttCnx,
      clientID: mqttOptions.clientID, qos: mqttOptions.QoS);
...
void publish(MqttClient c, MqttPubOptions mqttOptions) {
  c
      .publish(mqttOptions.topic, mqttOptions.payload, mqttOptions.messageID,
          mqttOptions.QoS, mqttOptions.retain)
      .then((m) {
    print("published !");
    print("Message ${m.messageID} published");
    if (mqttOptions.willTopic == null) {
      // if a WILL is defined, don't disconnect to have the will sent
      c.disconnect();
    }
  });
}
